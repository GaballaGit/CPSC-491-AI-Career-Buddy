...
function validateCreateProject(
  ...
  if (
    input.project_urls !== undefined &&
    !isStringArray(input.project_urls)
  ) {
    return {
      valid: false,
      field: 'project_urls',
  ...
  const dto: CreateProjectDto = {
    ...
    description,
    skills_demonstrated:
      input.skills_demonstrated.map((skill) =>
        skill.trim(),
      ),
  };

  if (input.project_urls !== undefined) {
    dto.project_urls = input.project_urls.map((url) =>
      url.trim(),
    );
  }
...
export const getProjects: RequestHandler = async (
  ...
  try {
    ...
    const projects =
      await projectRepository.findByUser(user.id);
